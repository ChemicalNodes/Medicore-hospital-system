...
class LoginScreen(tk.Tk):
    ...
    def _build(self):
        ...
        for label, value in [("SQLite","sqlite"),("MySQL","mysql"),("LibreOffice","libreoffice")]:
            tk.Radiobutton(
                db_frame, text=label,
    ...
    def _build_libreoffice_panel(self):
        c = self.colors
        tk.Label(self.panel, text="LIBREOFFICE .ODB FILE",
            bg=c["surface"], fg=c["muted"],
            font=("Courier", 9)).pack(anchor="w")
        row = tk.Frame(self.panel, bg=c["surface"])
        row.pack(fill="x", pady=(2, 0))
        self.odb_path = tk.StringVar()
        tk.Entry(row, textvariable=self.odb_path,
            bg=c["surface2"], fg=c["text"],
            font=("Courier", 11), relief="flat"
        ).pack(side="left", fill="x", expand=True, ipady=5)
        tk.Button(row, text="Browse",
            bg=c["surface2"], fg=c["text"],
            font=("Courier", 9), relief="flat", cursor="hand2",
            command=lambda: self.odb_path.set(
                filedialog.askopenfilename(
                    filetypes=[("LibreOffice Base","*.odb")]
                ) or self.odb_path.get())
        ).pack(side="right", padx=(4, 0))
